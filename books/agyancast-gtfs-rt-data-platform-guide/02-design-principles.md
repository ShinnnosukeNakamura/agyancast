...
まず、`agyancast` を作るときに最初に決めた「壊れにくくするための約束」を整理します。

本書のテーマは“モデル作り”ではなく、**GTFS-RTを継続取得して履歴として使える状態にすること**です。
そのために、データの置き方と責務分離（どこで何をするか）が重要でした。
...
今回のGTFS-RTは約15秒で更新されますが、`agyancast` の取得・変換は10分おきです。
これは「精度」よりも「継続運用」を優先した判断です。

- 15秒ポーリングはコストと運用が跳ねやすい（保存量、監視、リトライ、欠損処理が重くなる）
- まずは“傾向”を見る体験を成立させたい（完全なリアルタイムが主目的ではない）
- 将来、必要なら間隔は詰められる（設計を先に固める）
