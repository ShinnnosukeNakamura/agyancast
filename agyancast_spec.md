...
- `latest.json` / `latest_detail.json` は 10分ごとに更新（Lambda + EventBridge）
...
### 8.2.4 将来の Silver マート（parquet / Hive）

ローカル可視化で要件が満たせたら Silver 層に日次マートを追加する。

- 例: `s3://<data-bucket>/silver/mart/daily_delay/dt=YYYY-MM-DD/hour=HH/part-*.parquet`
- 例テーブル:
  - `dt` (string, partition)
  - `hour` (string, partition)
  - `mall_name` (string)
...
- 更新方式: 10分おきに **当日分の該当時刻バケットのみ** 再作成
