...
- 既存機能を維持:
  - 「今行くなら / 狙い目時間」カードは非表示
  - 並び順基準はコンパクト化（モール一覧ボタン群を廃止し、プルダウン + 現在地ボタン）
  - 一覧のソートもプルダウン化
  - 今日の推移グラフ（05-24時）
  - 推移グラフのY軸目盛りは固定表示（横スクロールしても見える）
  - 補助地図
...
- 空港リムジンバス向けダッシュボード。
- 3レイヤ表示:
  1. 全体遅延（阿蘇くまもと空港終点ベース、予測は非表示）
  2. 停留所別遅延（桜町BT ↔ 阿蘇くまもと空港、往復）
  3. 停留所別の当日推移グラフ（05-24時）
- `空港行き / 市内行き` トグルはタブ先頭に配置。
- トグルに合わせて「今日の遅延推移」「停留所別遅延」の両方を切替。
- 1時間後/3時間後の予測表示は非表示。
...
- 明示更新ボタンも併設。
- ヘッダーは1行固定: `熊本 混雑ナビ agyancast`（高さを抑え、表示領域を確保）
- `現在地を使う` は失敗理由を画面表示:
  - HTTPS要件
  - 権限拒否
  - タイムアウト
...
- 買物タブで「今行くなら / 狙い目時間」カードが非表示になり、基準/ソートがプルダウンで操作できる。
- 買物タブの「今日の混雑推移」でY軸目盛りが固定表示される。
- 通勤タブでセミコンテクノパークを1拠点表示し、不要KPI（1時間後/3時間後/速度サンプル）が非表示。
- 来熊タブで方向切替が先頭にあり、遅延推移と停留所表示が連動して切替される。
- 買物タブの既存機能が崩れない。
